...
def _download_one_file(args):
    url, file_path, callback, pbar, ignore_errors, head, log_level, parallel_downloads = args
    ...
    try:
        ...
        out = download_url(url, filename=file_path, progress_tracker=pbar, head=head)
        if callback:
            try:
                callback(out) # callback may take the downloaded file as an argument or no arguments
            except TypeError:
                callback()
        return out
    except Exception as e:
